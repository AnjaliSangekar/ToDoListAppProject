...
public class ListController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//pw.print("<input type='text' placeholder='Type a text' name='list'>  <button value='add'>Add</button>" );
		...
		int status = Listdao.addlist(ob);
		
		if(status > 0)
		{
			//pw.print("<script> alert('Added..') </script>");
			RequestDispatcher rd = request.getRequestDispatcher("List.html");
			rd.include(request, response);
		}
		else
		{
			//pw.print("<script> alert('Not Added..') </script>");
			RequestDispatcher rd = request.getRequestDispatcher("List.html");
			rd.include(request, response);
		}
